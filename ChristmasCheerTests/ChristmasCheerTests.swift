...
class ChristmasCheerTests: XCTestCase {
    ...
    func testUsersCrud() {
        let req = expectation(description: "network request")
//        Backend.makeUser(name: "teasdfst") { new in
//            do {
//                let ob = try new.get()
//                print("made user: \(ob)")
//                Backend.getUsers { result in
//                    do {
//                        let users = try result.get()
//                        print("got users: \(users)")
//                    } catch {
//                        XCTFail("oh noooo... \(error)")
//                    }
//                    req.fulfill()
//                }
//            } catch {
//                XCTFail("oh nooooo.. make \(error)")
//                req.fulfill()
//            }
//        }

        waitForExpectations(timeout: 25) { (error) in
            if let error = error { XCTFail("\(error)") }
            else { XCTFail("timeout") }
        }
